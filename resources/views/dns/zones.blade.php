        ...
        <div class="bg-white border-t border-b sm:border-l sm:border-r sm:rounded shadow mb-6">
            <div class="flex-grow flex flex-col bg-white border-t border-b sm:rounded sm:border shadow overflow-hidden">
                ...
                <div class="flex-grow flex px-6 py-3 text-grey-darker items-center border-b -mx-4">
                    ...
                    <div class="w-1/5 text-left text-grey italic">{{ $row->records->count() }}</div>
                    <div class="w-1/5 text-left text-grey text-sm italic">{{ (!empty($row->expire_date)) ? 'Exp: ' . $row->expire_date : 'Exp: unknown' }}</div>
                    <div class="w-1/5 mr-3 text-right">
                        <a href="{{ route('dns.records', $row->name) }}" class="flex-inline no-underline items-center justify-center cursor-pointer border-0 bg-grey-darkest hover:bg-green-dark text-white text-sm font-light rounded rounded-r-none py-1 px-2">
                        ...
                        <a href="{{ route('dns.zones.delete', $row->id) }}" class="flex-inline no-underline items-center justify-center cursor-pointer border-0 bg-grey-darkest hover:bg-red-dark text-white text-sm font-light rounded rounded-l-none py-1 px-2">
